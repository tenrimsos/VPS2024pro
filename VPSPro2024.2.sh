...
squidConfig() {
   ...
   if [[ "$AuthResponse" = 's' ]]; then
      ...
      echo "acl localhost src 127.0.0.1/32 ::1" >> /etc/squid3/squid.conf
      echo "acl to_localhost dst 127.0.0.0/8 0.0.0.0/32 ::1" >> /etc/squid3/squid.conf
      echo "acl SSL_ports port 443 143" >> /etc/squid3/squid.conf
