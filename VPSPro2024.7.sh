...
v2rayInstall() {
   ...
   /snap/bin/certbot --webroot -i nginx --agree-tos --hsts --staple-ocsp -d $v2rayHost -w /usr/share/nginx/html/
   # Abriendo puertos
   echo ""; tput sgr0 ; tput setaf 4
...
certbotInstall() {
   ...
   echo ""; tput sgr0 ; tput setaf 7
   # Instalando certificado
   /snap/bin/certbot --webroot -i nginx --agree-tos --hsts --staple-ocsp -d $certbotHost -w /usr/share/nginx/html/
   echo ""; tput sgr0 ; tput setaf 2
...
shadowsocksInstall() {
   ...
   read -p "Tipo de instalación? [1]: " -e -i n ssResponse
...
configIptables() {
   ...
   # wireguard
   iptables -A INPUT -p tcp --dport 41194 -j ACCEPT
   # webserver
   iptables -A INPUT -p tcp --dport 8000 -j ACCEPT
}
...
configFirewalld() {
   ...
   # BadVPN
   # squid
   ññfirewall-cmd --permanent --zone=public --add-port=8080/tcp
   # shadowscks
   ...
   # wireguard
   firewall-cmd --permanent --zone=public --add-port=41194/udp
   # webserver
