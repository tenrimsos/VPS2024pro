...
wireguardInstall() {
   ...
   #$wireguardExist = apt -qq list wireguard
   
   echo apt -qq list wireguard
   
   if [[ apt -qq list wireguard != "" ]]; then
      ...
      if [[ "$wireguardResponse" = 's' ]]; then
         ...
         #expect "Name [client]: "
         #printf "\41194\freehttp\\" | ./wireguard.sh
         
         #echo -n "Name [client]: "
         #read -r client
      
         #if [[ "$user" != '' ]]; then
         #   mv /root/"$user.conf" $user.conf
         #³fi
      else
         echo ""; tput sgr0 ; tput setaf 4
         echo "Ejecutando script para instalar StrongSwam IKEv2"; tput sgr0 ; tput setaf 7
...
portsOpen() {
   # Reabrir puertos despues de un reinicio
   # v2ray
   sudo iptables -I INPUT -p tcp --dport 443 -j ACCEPT
   sudo iptables -I INPUT -p tcp --dport 80 -j ACCEPT
   # hysteria2
