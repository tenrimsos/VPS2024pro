...
wireguardInstall() {
   ...
   if [[ "$wireguardResponse" = 's' ]]; then
      ...
      #sudo bash wireguard.sh <<ANSWERS n 41194 cliente 2 y ANSWERS
      sudo bash wireguard.sh
      #read -p "Cliente: " -e -i $user WIREGUARD_USER
      #read -p "Puerto: " -e -i 51820 WIREGUARD_PORT
      
      echo $unsanitized_client
      echo $port
      
      mv /root/"$unsanitized_client.conf" $unsanitized_client.conf
